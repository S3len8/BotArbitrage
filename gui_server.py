    ...
    async def fetch_trade_data(t):
        async with sem:
            ...
            try:
                ...
                cur_spread = ((sp - lp) / sp * 100) if sp > 0 else 0
                entry_spread = ((t.short_entry_price - t.long_entry_price) / t.short_entry_price * 100) if (t.short_entry_price and t.short_entry_price > 0) else 0

                item.update({
                    'cur_short_price': sp,
                    'cur_long_price': lp,
                    'cur_spread_pct': round(cur_spread, 2),
                    'entry_spread_pct': round(entry_spread, 2),
                ...
                if t.status == 'open' and (not s_pos_ok or not l_pos_ok):
                    ...
                    item['short_close_price'] = t.short_close_price
                    item['long_close_price'] = t.long_close_price
                    item['short_pnl_usd'] = t.short_pnl_usd
                    item['long_pnl_usd'] = t.long_pnl_usd
                    item['net_pnl_usd'] = t.net_pnl_usd
            except Exception as e:
...
def _t(t) -> dict:
    return {
        ...
        "short_qty": t.short_qty, "long_qty": t.long_qty,
        ...
        "short_close_time": str(t.short_close_time) if t.short_close_time else None,
        "long_close_time":  str(t.long_close_time) if t.long_close_time else None,
    }
